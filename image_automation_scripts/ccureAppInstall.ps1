...
<#
.DESCRIPTION
       This script installs MS SQL Server unattended from the executable image.
    It also installs necessary IIS features and runs a dashboard installer.
    The entire operation is recorded in a log file.
...
function Install-DashboardInstaller {
    param ()
    ...
    $command = "C:\Program Files\ccure\$ccureBuild\Copied\ISOImage\bin\dashboard.exe"
    # Define the arguments as an array
    $arguments = @(
        "/SOURCE:`"C:\Program Files\ccure\$ccureBuild\Copied\ISOImage`"",
